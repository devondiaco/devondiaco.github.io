class ScrollerVis {
  ...
  initVis() {
    ...
    // Identify the top 5 assignments with the longest time spent working
    vis.dataTop5 = [...vis.data].sort((a,b) => b.time - a.time).slice(0,5);
    const namesTop5 = vis.dataTop5.map(d => d.name);
    vis.data.forEach(d => {
    ...
    vis.xScale = d3.scaleLinear()
        ...
        .domain([0, d3.max(vis.dataTop5, d => d.time)]);
  ...
  step1() {
    ...
    // Highlight assignments rated as easy
    vis.rect.transition()
        .attr('opacity', 1)
        .attr('width', d => vis.config.cellWidth)
        .attr('height', d => vis.config.cellHeight)
  ...
  step2() {
    ...
    // Highlight assignments rated as difficult
    vis.rect.transition()
        .attr('opacity', 1)
        .attr('width', d => vis.config.cellWidth)
        .attr('height', d => vis.config.cellHeight)
  ...
  step3() {
    ...
    // Highlight the assignment with the longest time spent working
  ...
  step4() {
    ...
    vis.textG = vis.chart.selectAll('g.assignment-label')
        .data(vis.data.filter(d => d.rank >= 0), d => d.name)
      .join('g')
        .attr('class', 'assignment-label')
        .attr('opacity', 0)
        .attr('transform', d => `translate(${vis.config.yAxisWidth},${d.rank * (vis.config.barHeight + vis.config.barSpacing)})`);

    vis.textG.selectAll('text').remove();
